...
4. 관계 정의
    ...
    과계명은 모델러가 직접 작명을 해야하나 어려운 영역으로 보긴 힘듬
...
1. super type + 모든 sub Type 통합 (생성되는 테이블 수 : 1)
   하나의 테이블로 모든 속성을 넣는 형태로, 단점은 null값을 갖는 컬럼이 많이 질 수 있음
2. super type + sub type (생성되는 테이블 수 : sub type 개수만큼)
   sub type별로 테이블을 생성
3. super type, sub type 각각 생성(생성되는 테이블수 : super type, sub type 개수 만큼)
   자바의 클래스 상속과 유사
...
정규화 순서 : 1정규형 - 2정규형 - 3정규형 - BCNF 정규형 - 4정규형 - 5정규형
     순서가 정해져 있음. 3정규형은 1,2, 정규형이 끝난상태에서 진행
