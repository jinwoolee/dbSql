...
col | expression [as] alias ==> 별칭이 대문자로 인식됨
col | expression [as] "alias" ==> 별칭에 공백이나, 대소문자를 구분하고 싶을 때

SELECT 'TEST1' alias1, 'TEST2' AS alias2, 'TEST3' "alias3"
FROM dual;
...
SELECT *
FROM emp
WHERE TO_CHAR(hiredate, 'YYYYMMDD') BETWEEN '19800101' AND '19821231';
WHERE hiredate BETWEEN 
      TO_DATE('19800101', 'YYYYMMDD') AND TO_DATE('19821231', 'YYYYMMDD');
...
ename 컬럼으로 내림차순 정렬하고, 
만약에 동일한 ename이 있을경우 해당 데이터는 mgr 컬럼으로 정렬한다
...
1. SYSDATE
2. LAST-DAY ? LASTDAY ?? LAST_DAY(DATE)
