...
사번을 입력받아서, 해당 사원의 이름을 반화하는 FUNCTION
...
사원이름, 매니저 사원이름(관리자가 없을 때는 null로 표현)
...
오라클은 10번 인자로  getDeptName실행 했을 때 ACCOUNTING이라는 결과값을 기억(캐싱)
...
패지키 생성 방법 : java의 interface, class 사용 방법과 유사
...
trg_users_pass 트리거에 의해 users_history 테이블에
...
트리거
    장점(시스템 신규로 만드는 사람 SI) 
        : users테이블에 비밀번호를 바꾸는 로직(java 코드)은 작성을 해야한다
