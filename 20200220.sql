...
SELECT TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD'), 'YYYYMMDD') + ROUND(DBMS_RANDOM.value(-30, 30)) dt,
...
WHERE EXISTS ( SELECT 'X'
               FROM sem.gis_dt
               WHERE gis_dt.dt BETWEEN d AND  TO_DATE(TO_CHAR(d, 'YYYYMMDD') || '235959', 'YYYYMMDDHH24MISS'));
...
CREATE OR REPLACE PROCEDURE printdept_sem IS
v_deptno dept.deptno%TYPE;
v_dname dept.dname%TYPE;
...
CREATE OR REPLACE PROCEDURE printdept_p_sem(p_deptno IN dept.deptno%TYPE) IS
    v_dname dept.dname%TYPE;
    v_loc dept.loc%TYPE;
BEGIN
